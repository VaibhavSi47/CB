...
statecode_to_state = {
    "state_mappings": {'tt': 'Total', 'kl': 'Kerala', 'dl': 'Delhi', 'tg': 'Telangana', 'rj': 'Rajasthan',
                       'hr': 'Haryana', 'up': 'Uttar Pradesh', 'la': 'Ladakh', 'tn': 'Tamil Nadu',
                       'jk': 'Jammu and Kashmir', 'ka': 'Karnataka', 'mh': 'Maharashtra', 'pb': 'Punjab',
                       'ap': 'Andhra Pradesh', 'ut': 'Uttarakhand', 'or': 'Odisha', 'py': 'Puducherry',
                       'wb': 'West Bengal', 'ch': 'Chandigarh', 'ct': 'Chhattisgarh', 'gj': 'Gujarat',
                       'hp': 'Himachal Pradesh', 'mp': 'Madhya Pradesh', 'br': 'Bihar', 'mn': 'Manipur',
                       'mz': 'Mizoram', 'ga': 'Goa', 'an': 'Andaman and Nicobar Islands', 'jh': 'Jharkhand',
                       'as': 'Assam', 'ar': 'Arunachal Pradesh', 'tr': 'Tripura', 'ml': 'Meghalaya'}}
state_to_statecode = {
    "state_mappings": {'Total': 'tt', 'Kerala': 'kl', 'Delhi': 'dl', 'Telangana': 'tg', 'Rajasthan': 'rj',
                       'Haryana': 'hr', 'Uttar Pradesh': 'up', 'Ladakh': 'la', 'Tamil Nadu': 'tn',
                       'Jammu and Kashmir': 'jk', 'Karnataka': 'ka', 'Maharashtra': 'mh', 'Punjab': 'pb',
                       'Andhra Pradesh': 'ap', 'Uttarakhand': 'ut', 'Odisha': 'or', 'Puducherry': 'py',
                       'West Bengal': 'wb', 'Chandigarh': 'ch', 'Chhattisgarh': 'ct', 'Gujarat': 'gj',
                       'Himachal Pradesh': 'hp', 'Madhya Pradesh': 'mp', 'Bihar': 'br', 'Manipur': 'mn',
                       'Mizoram': 'mz', 'Goa': 'ga', 'Andaman and Nicobar Islands': 'an', 'Jharkhand': 'jh',
                       'Assam': 'as', 'Arunachal Pradesh': 'ar', 'Tripura': 'tr', 'Meghalaya': 'ml'}}
...
def get_dist_based_stat():
    ...
    for state in cases_by_dist.json().keys():
        for place, val in zip(cases_by_dist.json()[state]['districtData'].keys(),
                              cases_by_dist.json()[state]['districtData'].values()):
            case_dict[place] = {'confirmed': val['confirmed'], 'deceased': val['deceased'],
...
def get_testcenters_by_state(state):
    ...
    for res in facilities_json['resources']:
        if res['category'].lower() == 'CoVID-19 Testing Lab'.lower() and res['state'].lower() == state.lower():
             facilities.append(
                res['nameoftheorganisation'] + ", " + res['city'] + ", " + res['state'] + ", " + "Phone: " + res[
                    'phonenumber'])
...
def get_hospitals_by_state(state):
    ...
    for res in facilities_json['resources']:
        if res['category'].lower() == 'hospitals and centers' and res['state'].lower() == state.lower():
            facilities.append(
                res['nameoftheorganisation'] + ", " + res['city'] + ", " + res['state'] + ", " + res['phonenumber'])
    return facilities
...
def get_freefoods_by_city(city):
    facilities = []
    for res in facilities_json['resources']:
        if (res['category'].lower() == 'free food' or res['category'].lower() == 'community Kitchen') and res[
        ...
        try:
            ...
            if facility == "free food":
                ...
                if len(facilities_city) != 0:
                    ...
                    dispatcher.utter_message(
                        "Here is the address of the {} facilities in {}: {}".format(facility, location, allfacilities))
            ...
            if facility == "corona cases":
                ...
                if location.title() in cases_dict.keys():
                    ...
                    dispatcher.utter_message(
                        "Here is the statistics for {}: \n Confirmed Cases: {} Deceased: {} Recovered: {}".format(
                            location, cases['confirmed'], cases['deceased'], cases['recovered']))
                else:
            ...
            if facility == "email":
                ...
                if len(facilities_city) != 0:
                    allfacilities = "\n".join(facilities_city)
                    bodystr1 = "Here is the address of the {} facilities in {}: {}".format(facility, location,
                                                                                           allfacilities)
                elif len(facilities_state) != 0:
                    allfacilities = "\n".join(facilities_state)
                    bodystr1 = "Here is the address of the {} facilities in {}: {}".format(facility, location,
...
class UserForm(FormValidationAction):
    ...
    def submit(
        ...
        try: ##Define what the form has to do after all required slots are filled
...
def sendMail(emailid, name, body):
    ...
    s.login("jaiho.kumar777@gmail.com", "123654")
    ...
    message = 'Subject: {}\n\n{}'.format(subject, text)
    # sending the mail
    s.sendmail("jaiho.kumar777@gmail.com", emailid, message)
